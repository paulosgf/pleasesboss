...
redirectURI = "https://6b452916-b836-4123-b50b-518d368ec608-00-3l3sguw8yrcx6.riker.replit.dev/oauth"  #This could be different if you publicly expose this endpoint.
"""
...
def get_tokens(code):
    ...
    # Imprima o status e a resposta para debugar
    print("Response Status Code:", req.status_code)
    print("Response Text:", req.text)

    # Verifique se o status code é 200 (OK)
    if req.status_code == 200:
        results = req.json()

        # Certifique-se de que o 'access_token' está na resposta
        if 'access_token' in results:
            ...
            print("Refresh Token stored in session : ",
...
def get_tokens_refresh():
    ...
    payload = ("grant_type=refresh_token&client_id={0}&client_secret={1}&"
               "refresh_token={2}").format(clientID, secretID,
                                           session['refresh_token'])
    req = requests.post(url=url, data=payload, headers=headers)
...
def login():
    ...
    print("Redirecting to:",
          webex_authorize_url)  # Log da URL de redirecionamento
    return redirect(webex_authorize_url)
